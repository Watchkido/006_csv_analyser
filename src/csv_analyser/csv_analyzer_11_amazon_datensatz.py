...
def csv_info_extractor(csv_filepath):
    ...
    try:
        ...
        if total_nulls == 0:
            ...
            for col, null_count in null_counts.items():
                if null_count > 0:
                    ...
                    sample_null_rows = null_rows[:2] if null_rows else []
                    if sample_null_rows:
                        info_content.append(
                            f"   📋 Beispiel-Zeilen mit NULL in '{col}':"
                        )
                        # DataFrame-Format wie bei HEAD/TAIL
                        info_content.append(df.iloc[sample_null_rows].to_string(index=True, header=True))
        ...
        # AMAZON-ANALYSE: SPEZIELLE AUSWERTUNGEN UND BEISPIELCODE
        info_content.append("\n" + "=" * 80)
        info_content.append("📦 AMAZON-ANALYSE: SPEZIELLE AUSWERTUNGEN & BEISPIELCODE")
        info_content.append("=" * 80)
        info_content.append("")
        ...
        if all(col in df.columns for col in ['Qty', 'Amount']):
            ...
            umsatz_pro_bestellung = df_valid.groupby('Order ID')['Amount'].sum().mean() if 'Order ID' in df.columns else None
            info_content.append(f"Gesamtumsatz (nur nicht-stornierte Bestellungen): {gesamtumsatz:,.2f}")
            info_content.append(f"Durchschnittlicher Bestellwert: {durchschnittspreis:,.2f}")
            if umsatz_pro_bestellung is not None:
                info_content.append(f"Umsatz pro Bestellung: {umsatz_pro_bestellung:,.2f}")
        # 2. Top-Kategorien und Top-SKUs
        if 'Category' in df.columns and 'Qty' in df.columns:
            top_cat = df.groupby('Category')['Qty'].sum().sort_values(ascending=False).head(10)
        ...
        # 3. Zeitliche Trends
        if 'Date' in df.columns and 'Qty' in df.columns and 'Amount' in df.columns:
            try:
                df['Date'] = pd.to_datetime(df['Date'], errors='coerce')
                df['Monat'] = df['Date'].dt.to_period('M')
                df_monatlich = df.groupby('Monat').agg({'Qty': 'sum', 'Amount': 'sum'})
                info_content.append("\nMonatliche Verkäufe & Umsatz:")
                info_content.append(df_monatlich.to_string())
            except Exception as e:
                info_content.append(f"Fehler bei Zeitreihenanalyse: {e}")
        # 4. Geografische Analysen
        if 'ship-city' in df.columns and 'Amount' in df.columns:
            umsatz_stadt = df.groupby('ship-city')['Amount'].sum().sort_values(ascending=False).head(10)
        ...
        if all(col in df.columns for col in ['Category', 'Size', 'Qty']):
            try:
                groessen = df.groupby(['Category', 'Size'])['Qty'].sum().unstack().fillna(0)
                info_content.append("\nGrößen-Analyse nach Kategorie:")
                info_content.append(groessen.to_string())
            except Exception as e:
        ...
        if 'Courier Status' in df.columns and 'Status' in df.columns:
            cancelled_courier = df[df['Courier Status'].isnull()]['Status'].value_counts()
            info_content.append("\nStatus bei fehlendem Courier Status:")
            info_content.append(cancelled_courier.to_string())
        # 10. ASIN → SKU → Style → Kategorie
        if all(col in df.columns for col in ['ASIN', 'SKU', 'Style', 'Category', 'Qty']):
            asin_sku = df.groupby(['ASIN', 'SKU', 'Style', 'Category'])['Qty'].sum().sort_values(ascending=False).head(10)
...
def erstelle_auswertungsdiagramme(csv_path: str) -> str:
    ...
    with PdfPages(pdf_path) as pdf:
            ...
            for i in range(0, n, 8):
                ...
                for j in range(8):
                    ...
                    try:
                        ...
                        # Fehler unterdrücken, nur "Keine Daten" anzeigen
                        ax.text(0.5, 0.5, "Keine Daten", ha="center", va="center", fontsize=8)
                        ax.axis('off')
                    # Animiertes Zeichen anzeigen
                    sys.stdout.write(f"\rDiagramm wird erstellt... {anim[anim_idx % len(anim)]} ")
                    sys.stdout.flush()
                    anim_idx += 1
                    time.sleep(0.05)
                plt.tight_layout()
                pdf.savefig(fig)
                plt.close(fig)
            sys.stdout.write("\rDiagramm wird erstellt... Fertig!\n")
            sys.stdout.flush()
        ...
        if len(numeric_cols) > 0:
            plot_grid([df[col] for col in numeric_cols],
                      [f"Boxplot: {col}" for col in numeric_cols], kind="box")
        ...
        if len(numeric_cols) >= 2:
            ...
            # Statt alle Punkte direkt zu plotten, als PNG speichern und einfügen
            import tempfile
            for idx in range(len(scatter_x)):
                xcol, ycol = scatter_x[idx], scatter_y[idx]
                title = scatter_titles[idx]
                df_pair = df[[xcol, ycol]].dropna()
                if len(df_pair) == 0:
                    ...
                    sample_size = min(5000, len(df_pair))
                    df_sample = df_pair.sample(n=sample_size, random_state=42)
                    fig, ax = plt.subplots(figsize=(8, 6))
                    ax.scatter(df_sample[xcol], df_sample[ycol], alpha=0.5, s=5)
                    ax.set_xlabel(xcol)
                    ax.set_ylabel(ycol)
                    ax.set_title(title)
                    plt.tight_layout()
                    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmpfile:
                        fig.savefig(tmpfile.name, dpi=120)
                        plt.close(fig)
        ...
        max_points = 1000  # Maximal 1000 Werte pro Zeitreihe für schnellere Plots
        for zeit_col in zeit_spalten:
            try:
                ...
                for num_col in numeric_cols:
                    # Nur die ersten max_points Werte verwenden
                    x_vals = zeit_serie.iloc[:max_points]
                    y_vals = df[num_col].iloc[:max_points]
                    time_x.append(x_vals)
                    time_y.append(y_vals)
        ...
        if geschlechts_spalten and len(numeric_cols) > 0:
            ...
            for gcol in geschlechts_spalten:
                for ncol in numeric_cols:
                    # Nur wenn mind. 2 Gruppen vorhanden
            ...
            def plot_violin_grid(plots, titles):
                ...
                for i in range(0, n, 8):
                    ...
                    for j in range(8):
                        ...
                        try:
                            gcol, ncol = plots[idx]
                            # Humorvoller Kommentar: "Violine statt Geige – für die feinen Unterschiede!"
                            sns.violinplot(x=df[gcol], y=df[ncol], ax=ax, inner="box", cut=0)
                            ax.set_xlabel(gcol)
                            ax.set_ylabel(ncol)
                            ax.set_title(titles[idx], fontsize=9)
                        except Exception as e:
